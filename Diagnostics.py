...
class Diagnostics:  # Make this a subclass of ht.DNDarray?
    def __init__(self, Mask, Perm, Poro, Sstorage, Ssat, Sres, Nvg, Alpha, Mannings, Slopex, Slopey, Dx, Dy, Dz, Dzmult, Nx, Ny, Nz, Terrainfollowing, Split3D, Split2D=None):
        self.Mask             = Mask
        self.Perm             = Perm
        self.Poro             = Poro
        ...
        self.Split2D          = Split2D if Split2D is not None else Split3D
...
#         Toplayerpress[ht.nonzero(toplayer < 0)] = fill_val
    ...
    def _NetLateralOverlandFlow(self, overland_flow_x, overland_flow_y):
        ...
        flow_west[:, 0:1] = ht.where(overland_flow_x[:,0:1]<0.0, overland_flow_x[:,0:1],Nix[:,0:1])
        ...
        flow_south[0:1] = ht.where(overland_flow_y[0:1,:]<0, overland_flow_y[0:1,:], Nix[0:1,:])
